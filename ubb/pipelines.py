...
class UbbPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('ubb.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `ubb`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM ubb WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO ubb (date, title, link, content)"
